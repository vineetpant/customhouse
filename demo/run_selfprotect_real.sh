#!/usr/bin/env bash
# Customhouse self-protection, proven against a REAL MCP server rather than the
# bundled mock: the official @modelcontextprotocol/server-filesystem.
#
# CUSTOMHOUSE_HOME is deliberately placed inside the directory that server is
...
# The server's allowed directory. Customhouse's own state lives inside it — that is
