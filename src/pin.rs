...
//! model context, so Customhouse pins each tool definition at first sight and
...
///
/// Lives at `<customhouse-home>/pins.json`, inside the directory the §3 gate
...
impl PinStore {
    /// Open the store at `<customhouse-home>/pins.json` (production).
    pub fn open() -> Self {
    ...
    pub fn open_in(home: &Path) -> Self {
        ...
        let data = Self::load(&path).unwrap_or_else(|e| {
            eprintln!("customhouse: pin store unreadable, starting empty: {e}");
            PinData::default()
        });
        Self { path, data }
    }
    ...
    /// the next connect. This is what `customhouse repin <server>` does. Returns
